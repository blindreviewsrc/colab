...
class VERILOGPlugin(Magics):
    ...
    def print_verilog(self, line, cell):
        ...
        if "-top" not in line: 
            args = "yosys -Q -T -q -s /content/colab/verilog/script.ys"
        else:
            args = '/content/colab/verilog/yosys_command.sh ' + line.replace("-top","").replace(" ","") + ' code.v'
            print(args)
        
        colab.compile("iverilog", cell, "code.v", "code.out")
        colab.command_line(args)
        colab.command_line('/content/colab/verilog/netlistsvg/bin/netlistsvg.js output.json --skin /content/colab/verilog/netlistsvg/lib/default.svg')
    ...
    def waveform(self, line, cell):
        ...
        import sys
        sys.path.insert(0,'.')
        from colab.verilog.vcd_parser.vcd_plotter import VcdPlotter
