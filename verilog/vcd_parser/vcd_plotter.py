...
from colab.verilog.vcd_parser.vcd_parser import VcdParser
...
class VcdPlotter():
    ...
    def plot(self, op_dict, signals_list: [str], start_time: int,
        ...
        for sig in signals_list:
            ...
            if len(s) == 1:
                data[s[0]] = self.convert(op_dict, base,
                                          raw[start_time:stop_time],
                                          self.__signal_store.signals[
                                              id].width)
            elif len(s) == 2:
                s[1] = s[1].lower()
                data[s[0]] = self.convert(op_dict, s[1],
                                          raw[start_time:stop_time],
                                          self.__signal_store.signals[
                                              id].width)
            else:
                ...
                if value[2:] == '0':
                    ...
                    if value != value_before:
                        if 'x' in value_before:
                            plt.axvline(x=j, ymin=start_vert, ymax=end_vert,
                                        color='r')
                        elif 'z' in value_before:
                            plt.axvline(x=j, ymin=start_vert,
                    ...
                    if j == 0 or value != value_before:
                        if j > 0 and 'x' in value_before:
                            ...
                            plt.axvline(x=j, ymin=start_vert + 0.25 / size_y,
                                        ymax=end_vert, color='g')
                        else:
                            plt.axvline(x=j, ymin=start_vert, ymax=end_vert,
                                        color='g')
                        plt.text(j + 0.08, i + 0.1, value[2:], fontsize=14)
                    plt.axhline(y=i + 0.5, xmin=start_hor, xmax=end_hor,
                                color='g')
                elif 'z' in value:
                    if j > 0 and value != value_before:
                        if 'x' in value_before:
                            ...
                            plt.axvline(x=j, ymin=start_vert, ymax=end_vert,
                                        color='g')
                    plt.axhline(y=i + 0.25, xmin=start_hor, xmax=end_hor,
                                color='y')
                elif 'x' in value:
                    if j == 0 or value != value_before:
                        plt.axvline(x=j, ymin=start_vert, ymax=end_vert,
                        ...
                        if len(value) > count * 7:
                            plt.text(j + 0.08, i + 0.1, value[:count * 7 - 3] +
                                     "...", fontsize=14, color='r')
                        else:
        ...
        for i in range(len(data)):
            ...
            if base == 'dec':
                ...
                if data[i] == 'x' or data[i] == 'z':
                    ...
                    data[i] = 'b\'' + format(int(data[i], 2), '0%db' %
                                             data_width)
            elif base == 'hex':
                if data[i] == 'x' or data[i] == 'z':
                    data[i] = 'h\'' + (data[i] * (data_width // 4))
                else:
                    data[i] = 'h\'' + format(int(data[i], 2), '0%dx' %
                                             (data_width // 4))
            elif base == 'dec2':
                if data[i] != 'x' and data[i] != 'z':
